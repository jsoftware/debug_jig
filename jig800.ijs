NB. Version 2.0 for this add on - QTide tooltips used - J807 passed testfile load '/users/bobtherriault/j64-807-user/projects/enhanced/runjig901.ijs'
...
if. b do. yc=.,.(msk=.a:~: ,lit) # rsfiy lit=. boxutf y 
      ...
      else. yc=.,.( ([ S:0 (# each each lit) ) # (18 * _1 |.!.0 +/\)) @:;@: (;@:(}: , >:@:;each@:{:) each) @: (+/@:((10;13)e.{:) each each) lit_base_=:lit=. unb lc boxutf y 
...
if. s do. (19.5 + >./  xw +&, xc);(32 + (18 * ({: ; lit) e. 10;13)+{: ,yc);tm NB. width height character string
    else. tt=.lbtt '<title> Literal    ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth: ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
...
if. s do. (21.5+ >./  xw +&, xc);(32 + (18 * ({: ; uni2) e. 10;13)+{: ,yc); tm
    else. tt=.lbtt '<title> Unicode      ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth: ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
...
if. s do. (21.5+ >./  xw +&, xc);(32 + (18 * ({: ; uni4) e. 10;13)+{: ,yc); tm
    else. tt=.lbtt '<title> Unicode4    ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth: ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
...
tt=. lbtt '<title> Label Type: ',"1(;"1 slt),"1('     ',LF,' Index: '),"1(;"1(":each ,. , 6 s: y)),"1(' ',LF,' Total Symbols Assigned: '),"1(": 0 s: 0),"1' </title>'
tm=. '<svg x="';"1 sx ;"1'" y="';"1 sy ;"1'"><rect class=" ',"1((('unicode';'unicode4')i.slt){::'su';'sv';'sl'),"1'" width="',"1 (": ,. , sw ),"1'" height="',"1 (": ,. , sh),"1'" rx="3">',"1 tt ,"1'</rect>',"1 '<text  class="s" x="2" y="18" >`</text>',"1'"><svg>',"1  (>ysvg=. ,. {. > , <"1 > , &.:> {: each ysvg),"1 '</svg></svg>' NB. red background of symbol last text element is ` stroke
tt=. lbtt '<title> Symbol     ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth: ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
tm=. ;": each '<svg  x="4" y="8">';"1  tm ,"1<'</svg>'
'<rect class="';((1-: {. $ y){'jd');'" width="';(sW);'" height="';(sH);'" rx="6">',tt,'</rect>',tm
)

bxsvg=: 4 : 0 NB. boxed cases - sends contents through the process and boxes those results
'depth s b path'=. x
depth=.>: depth
ysvg=.  (<"1 ((depth ; s ; 1) ,"1 0 (path , ';',~": )each <"1 ''''''"_^:(1-:#@$) ($ $ (#: i.@:(*/))@$)  y))vg &> y  
...
tt=. lbtt '<title> Box         ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth: ',(": <:depth),'        ',LF,' Path: ',(}: path))),' </title>'
...
boosvg=: 4 : 0 NB. literal case
'depth s b path'=. x
yc=.,.( #@> # 18 * i.@#) num=.  < ;._2 a.&i. each  (0 100000 0 500) 5!:30 y 
xw=. ; xw [ xc=. ,.  ; 0&,@}:@(+/\) each xw=.(9.59"_)"0  each num=. num -.a:
tt=.lbtt '<title> Boolean    ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth:  ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
tm=. '<rect  x="',"1(": xc),"1'" y="',"1(":yc),"1'" width="',"1 (": ,. , xw),"1'"></rect>'
vals=. ,. ,{&a. > num 
tm=. ,tm  ,"1 '<text class="b" x="',"1(cnv xc+4.5),"1'" y="',"1(cnv yc+20),"1'">',"1 vals ,"1 '</text>'
'<rect class="';((1-: {. $ y){'jd');'" width="';(8 + >./  xw +&, xc);'" height="';(32 + {: ,yc);'" rx="6">',tt,'</rect>',tm 
)

intsvg=: 4 : 0 NB. literal case
'depth s b path'=. x
yc=.,.( #@> # 18 * i.@#) num=.  < ;._2 a.&i. each  (0 100000 0 500) 5!:30 y 
xw=. ; xw [ xc=. ,.  ; 0&,@}:@(+/\) each xw=.(9.59"_)"0  each num=. num -.a:
tt=.lbtt '<title> Integer    ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth:  ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
tm=. '<rect  x="',"1(": xc),"1'" y="',"1(":yc),"1'" width="',"1 (": ,. , xw),"1'"></rect>'
vals=. ,. ,{&a. > num 
tm=. ,tm  ,"1 '<text class="i" x="',"1(cnv xc+4.5),"1'" y="',"1(cnv yc+20),"1'">',"1 vals ,"1 '</text>'
'<rect class="';((1-: {. $ y){'jd');'" width="';(8 + >./  xw +&, xc);'" height="';(32 + {: ,yc);'" rx="6">',tt,'</rect>',tm 
)

extsvg=: 4 : 0 NB. literal case
'depth s b path'=. x
yc=.,.( #@> # 18 * i.@#) num=.  < ;._2 a.&i. each  (0 100000 0 500) 5!:30 y 
xw=. ; xw [ xc=. ,.  ; 0&,@}:@(+/\) each xw=.(9.59"_)"0  each num=. num -.a:
tt=.lbtt '<title> Extended   ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth:  ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
tm=. '<rect  x="',"1(": xc),"1'" y="',"1(":yc),"1'" width="',"1 (": ,. , xw),"1'"></rect>'
vals=. ,. ,{&a. > num 
tm=. ,tm  ,"1 '<text class="e" x="',"1(cnv xc+4.5),"1'" y="',"1(cnv yc+20),"1'">',"1 vals ,"1 '</text>'
'<rect class="';((1-: {. $ y){'jd');'" width="';(8 + >./  xw +&, xc);'" height="';(32 + {: ,yc);'" rx="6">',tt,'</rect>',tm 
)

flosvg=: 4 : 0 NB. literal case
'depth s b path'=. x
yc=.,.( #@> # 18 * i.@#) num=.  < ;._2 a.&i. each  (0 100000 0 500) 5!:30 y 
xw=. ; xw [ xc=. ,.  ; 0&,@}:@(+/\) each xw=.(9.59"_)"0  each num=. num -.a:
tt=.lbtt '<title> Floating    ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth:  ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
tm=. '<rect  x="',"1(": xc),"1'" y="',"1(":yc),"1'" width="',"1 (": ,. , xw),"1'"></rect>'
vals=. (,.;  {&a. > num) rplc"1 'e';'<tspan class="fa">e</tspan>'
tm=. ,tm  ,"1 '<text class="f" x="',"1(cnv xc+4.5),"1'" y="',"1(cnv yc+20),"1'">',"1  vals ,"1 '</text>' 
'<rect class="';((1-: {. $ y){'jd');'" width="';(8 + >./  xw +&, xc);'" height="';(32 + {: ,yc);'" rx="6">',tt,'</rect>',tm 
)

ratsvg=: 4 : 0 NB. literal case
'depth s b path'=. x
yc=.,.( #@> # 18 * i.@#) num=.  < ;._2 a.&i. each  (0 100000 0 500) 5!:30 y 
xw=. ; xw [ xc=. ,.  ; 0&,@}:@(+/\) each xw=.(9.59"_)"0  each num=. num -.a:
tt=.lbtt '<title> Rational    ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth:  ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
tm=. '<rect  x="',"1(": xc),"1'" y="',"1(":yc),"1'" width="',"1 (": ,. , xw),"1'"></rect>'
vals=. (,.;  {&a. > num) rplc"1 'r';'<tspan class="ra">r</tspan>'
tm=. ,tm  ,"1 '<text class="r" x="',"1(cnv xc+4.5),"1'" y="',"1(cnv yc+20),"1'">',"1  vals ,"1 '</text>' 
'<rect class="';((1-: {. $ y){'jd');'" width="';(8 + >./  xw +&, xc);'" height="';(32 + {: ,yc);'" rx="6">',tt,'</rect>',tm 
)

comsvg=: 4 : 0 NB. literal case
'depth s b path'=. x
yc=.,.( #@> # 18 * i.@#) num=.  < ;._2 a.&i. each  (0 100000 0 500) 5!:30 y 
xw=. ; xw [ xc=. ,.  ; 0&,@}:@(+/\) each xw=.(9.59"_)"0  each num=. num -.a:
tt=.lbtt '<title> Complex    ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';('    ',LF,LF,' Depth:  ',(":depth),'    ',LF,' Path: ',(}: path))),' </title>'
tm=. '<rect  x="',"1(": xc),"1'" y="',"1(":yc),"1'" width="',"1 (": ,. , xw),"1'"></rect>'
vals=. (,.;  {&a. > num) rplc"1 'j';'<tspan class="ja">j</tspan>'
tm=. ,tm  ,"1 '<text class="c" x="',"1(cnv xc+4.5),"1'" y="',"1(cnv yc+20),"1'">',"1  vals ,"1 '</text>' 
'<rect class="';((1-: {. $ y){'jd');'" width="';(8 + >./  xw +&, xc);'" height="';(32 + {: ,yc);'" rx="6">',tt,'</rect>',tm 
)
