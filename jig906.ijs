NB. Version 2.0 for this add on  903 - QTide tooltips used 
...
ndx2=. ".saved_select
wd 'pclose;' 
 if. (a:,a:,a:) -: ndx1{titlelist do. return. end. NB. blank line selected
 if. _2 ~: ndx1 + ndx2 do.  NB. nothing selected show nothing just exit
  if. ndx1 ~: _1 do.(_2 {. <;. _1 ; 1 { ndx1 { titlelist) showjig ndx1 {handlist end. NB. show current selected with information stored in titlelist
  if. ndx2 ~: _1 do.(_2 {. <;. _1 ; 1 { (ndx2 + 10) { titlelist) showjig ndx2 {savelist end.  NB. show saved selected with information stored in titlelist
 end.
)
showjig=: 4 : 0
 's f'=.x
 wd 'psel *',y
 wd 'set ', ((>(10 20 50 100 200 300 400 i.(".}:s)){'xs';'s';'sm';'m';'ml';'l';'xl')),' value 1;' NB. required in order to recapture the size button settings on display
 wd 'set ', f ,' value 1;' NB. required in order to recapture the font button settings on display
 wd 'setp visible 1;'
 wd 'set w1 focus;'
 wd 'pshow;'
)
...
cocurrent > {: x
if. >{. x do. vobj_jig_=. findline_jig_ y  else. vobj_jig_=.y  end. NB. if true then process current line or programmatic, if false then just a window redraw for size or font don't want to go back to the line.
...
select. s
 ...
 case.'integer';'integer1';'integer2';'integer4'       do.   x intsvg y 
 case.'extended'                                       do.   x extsvg y 
 case.'floating';'floating2';'floating4';'floating16'  do.   x flosvg y
...
if. s do. 34;32;'<rect class="syb" x="15.5" y="7" width="14.39" rx="2">',(tooltip ' Empty shape '),'</rect><text font-family="',FONT,'" font-size="0.8em" x="18.5" y="20"> </text>' NB. case for s: 0{a.  which has shape 0
      else. tt=. ' ',(; (('sparse boolean';'sparse integer';'sparse floating';'sparse complex';'sparse boxed';'literal';'unicode';'unicode4';'symbol';'boxed';'boolean';'integer';'integer1';'integer2';'integer4';'extended';'floating';'floating2';'floating4';'floating16';'rational';'complex') i. <typ){ ('Sparse Boolean';'Sparse Integer';'Sparse Floating';'Sparse Complex';'Sparse Boxed';'Literal';'Unicode';'Unicode4';'Symbol';'Box';'Boolean';'Integer';'Integer1';'Integer2';'Integer4';'Extended';'Floating';'Floating2';'Floating4';'Floating16';'Rational';'Complex')),LF,' Shape: ',(":$y),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
tt=. ' Sparse ',(;(1<#2 $. y){'Index: ';'Indices: '),(": 2$.y),LF,' Array Shape: ',(": $ y)
tm=. '<rect  rx="6" stroke="none" fill="#fff" x="7" y="9" width="',(": sep),'" height="',(": 2 + 17 * # m),'"',(tooltip tt),((": 11.5 );'i') nmercal  ind
tt=. ' Non-Sparse Entries: ',(": 7 $. y),LF,' Value Type: ',(; (('sparse boolean';'sparse integer';'sparse floating';'sparse complex';'sparse boxed') i. <x){ ('Boolean';'Integer';'Floating';'Complex';'Boxed')),LF,' Sparse Element: ',se
...
tt=. ' ',(; (('sparse boolean';'sparse integer';'sparse floating';'sparse complex';'sparse boxed') i. <x){ ('Sparse Boolean';'Sparse Integer';'Sparse Floating';'Sparse Complex';'Sparse Boxed')),LF,' Array Shape: ', (": $ y)
...
unb=:(mask (#^:_1)!.(<a:) ,)
tooltip=: (' onclick="alert(''' , (3}. [:; (<' \n ') ,"0 [: <;._1 LF&,) ,  ''')" ><title>' ,  , & '</title></rect>')"1

nonbreak=:(a.{~ ])@;@((<194 160) I.@((<32)= ])"1 } (a.&i. each))"1
...
if. s do.   tt=. nonbreak ' UTF-8: ',"1  (encode boxutf y) 
      else. tooltip tt=. nonbreak' UTF-8: ',"1  (encode boxutf y) ,"1 LF,"1' Path: ' ,"1 path ,"1  (newpath  boxutf y)  end.
...
if. s do. (19.5 + >./  xw +&, xc);(32 + (18 * ({: ; lit) e. 10;13)+{: ,yc);tm NB. width height character string
    else. tt=.' Literal  ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
if. b do. yc=. ,. (msk=.a:~: ,uni2) # rsfiy uni2=. boxuni y 
          ...
          xwv=.": ,. xw=. ;xw  end.
if. s do.   tt=.nonbreak ' Unicode: ',"1  (encode  boxuni y)
      else. tt=.nonbreak ' Unicode: ',"1  (encode  boxuni y),"1 LF,"1 ' Path: ' ,"1 path ,"1 (newpath boxuni y)  end.
...
if. s do. (21.5+ >./  xw +&, xc);(32 + (18 * ({: ; uni2) e. 10;13)+{: ,yc); tm
    else. tt=. ' Unicode  ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
if. b do. yc=. ,. (msk=.a:~: ,uni4) # rsfiy  uni4=.  boxuni y 
          ...
          xwv=. ": ,. xw=.; xw  end.  
if. s do.   tt=. nonbreak ' Unicode4: ',"1  (encode  boxuni y)
      else. tt=. nonbreak ' Unicode4: ',"1  (encode  boxuni y),"1 LF,"1 ' Path: ' ,"1 path ,"1 (newpath boxuni y)  end. 
...
if. s do. (21.5+ >./  xw +&, xc);(32 + (18 * ({: ; uni4) e. 10;13)+{: ,yc); tm
    else. tt=.' Unicode4 ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
tt=. '  Label Type: ',"1(;"1 slt),"1(LF,'  Index: '),"1(;"1(":each ,. , 6 s: y)),"1(LF,'  Total Symbols Assigned: '),"1(": 0 s: 0),"1(LF,'  Path: ') ,"1 path ,"1 (newpath y) NB.,"1 '</h1> '
...
tt=. ' Symbol     ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path))) NB.,' </h1>'
...
tt=.  ' Box  ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(": <:depth),LF,' Path: ',(}: path)))
tm=. ; ": each '<svg  x="';"1((4*b)+ 4+ bx);"1'" y="';"1(17+by);"1'" width="';"1 bw;"1'" height="';"1 bh;"1'">';"1  tm ,"1<'</svg>'
'<rect class="';((1-: {. $ y){'jd');'" width="';(bW- 8 * -. b);'" height="';(bH);'" rx="6" ',(tooltip tt),tm
)


boosvg=: 4 : 0 NB. numeric case - boolean  
'depth s b path'=. x 
tt=. ' Boolean  ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
'depth s b path'=. x 
tt=. ' I',(}.datatype y),' ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
'depth s b path'=. x 
tt=. ' Extended  ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
'depth s b path'=. x 
tt=. ' F',(}.datatype y),' ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
'depth s b path'=. x 
tt=. ' Rational  ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
...
comsvg=: 4 : 0 NB. numeric case - boolean  
'depth s b path'=. x 
tt=. ' Complex  ',LF,' Shape: ',((''-:  $ y){::((":  $ y);'atom')),(; b{'';(LF,LF,' Depth: ',(":depth),LF,' Path: ',(}: path)))
